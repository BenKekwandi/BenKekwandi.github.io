    ...
    <nav class="sidebar close">
        ...
        <div id="root">
            <table>
                    ...
                    <th align="center">Employee first name</th>
                    <th align="center">Employee last name</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php 
                            $id1=$requests[$n]['employee_id'];
                                $qr="select * from employees where employee_id=$id1";
                                $rs=mysqli_query($con,$qr);
                                if($rs)
                                {
                                    if($rs && mysqli_num_rows($rs)>0)
                                    {
                                        $that_employee=mysqli_fetch_assoc($rs);
                                    }
                                }
                                echo $that_employee['employee_firstName'];
                            ?>
                        </td>
                        <td>
                            <?php 
                            $id2=$requests[$n]['employee_id'];
                            $qr="select * from employees where employee_id=$id2";
                            $rs=mysqli_query($con,$qr);
                            if($rs)
                            {
                                if($rs && mysqli_num_rows($rs)>0)
                            ...
                            echo $that_employee['employee_lastName'];
                            ?>  
                        </td>
                        <td><?php echo $requests[$n]['request_content']?></td>
                        <td>
                        <?php echo $requests[$n]['request_status']?>
                        <form id="stat" action="requests_admin.php" method="post">
                        <textarea type="text" name="feedback" id="feedback" placeholder="Feedback of the request"></textarea>
                        <input type="submit" id="cfr" value="Send">
                        </form>
                        </td>
                    <?php } ?>
                    </tr>
